            ...
            if (result.Failed)
            {
...
#elif XUNIT
                        throw new Xunit.Sdk.AssertActualExpectedException(result.Specification.Then, result.ButException.Value, writer.ToString());
#endif
                    }
                }
                ...
                if (result.ButResult.HasValue)
                {
                    using (var writer = new StringWriter())
                    {
                        ...
                        throw new NUnit.Framework.AssertionException(writer.ToString());
#elif XUNIT
                        throw new Xunit.Sdk.AssertActualExpectedException(result.Specification.Then, result.ButResult.Value, writer.ToString());
#endif
                    }
                }
                ...
                if (result.ButEvents.HasValue)
                {
                    using (var writer = new StringWriter())
                    {
                        ...
                        throw new NUnit.Framework.AssertionException(writer.ToString());
#elif XUNIT
                        throw new Xunit.Sdk.AssertActualExpectedException(result.Specification.Then, result.ButEvents.Value, writer.ToString());
#endif
                    }
                }
            }
            ...
            if (result.Failed)
            {
                ...
                if (result.ButResult.HasValue)
                {
                    using (var writer = new StringWriter())
                    {
                        ...
                        throw new NUnit.Framework.AssertionException(writer.ToString());
#elif XUNIT
                        throw new Xunit.Sdk.AssertActualExpectedException(result.Specification.Throws, result.ButResult.Value, writer.ToString());
#endif
                    }
                }
            }
